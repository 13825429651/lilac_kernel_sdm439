 ...
 */
unsigned int sysctl_sched_wakeup_granularity = 450000UL;
unsigned int normalized_sysctl_sched_wakeup_granularity = 450000UL;
 ...
 */
unsigned int sysctl_sched_capacity_margin = 1280; /* ~5% margin */
unsigned int sysctl_sched_capacity_margin_down = 1280; /* ~15% margin */
				   ...
				   struct find_best_target_env *fbt_env)
{
	...
	*backup_cpu = -1;
	...
	if (target_cpu != -1 && !idle_cpu(target_cpu) &&
		...
		if (curr_tsk && schedtune_task_boost(curr_tsk)) {
			target_cpu = best_idle_cpu;
		}
	}
