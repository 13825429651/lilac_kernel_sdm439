...
function push() {
    ...
    curl -F document=@$ZIP "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendDocument" \
        ...
        -F "parse_mode=html" \
        -F caption="Build took $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s). | For <b> SDM439 series </b> | <b>$(${GCC}gcc --version | head -n 1 | perl -pe 's/\(http.*?\)//gs' | sed -e 's/  */ /g')</b>"
}
# Fin Error
function finerr() {
    curl -s -X POST "https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage" \
...
function zipping() {
    ...
    zip -r9 CY-4.9-SDM439-${TIMESTAMP}.zip *
    cd ..
}
sendinfo
compile
